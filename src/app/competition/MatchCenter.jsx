...
/** Mini Line Chart SVG para historial de ALTTEZ Score */
      ...
      <div style={{ padding: "14px 16px" }}>
        ...
        <div style={{ marginBottom: 10 }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 4 }}>
            <span style={{ fontSize: 8, textTransform: "uppercase", letterSpacing: "1.5px", color: C.textMuted }}>ALTTEZ Score</span>
            <span style={{ fontSize: 20, fontWeight: 900, color: scoreColor }}>{alttezScore.toFixed(1)}</span>
          </div>
          {/* Barra de progreso */}
          <div style={{ height: 3, background: "rgba(255,255,255,0.08)", borderRadius: 2, overflow: "hidden" }}>
            <motion.div
              initial={{ width: 0 }}
              animate={{ width: `${(alttezScore / 10) * 100}%` }}
  ...
  return (
    <div>
      ...
      <div className="mc-analytics-grid" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, marginBottom: 16 }}>
        ...
        {/* Historial de ALTTEZ Score */}
        <div style={{ background: "rgba(255,255,255,0.03)", border: `1px solid ${C.border}`, padding: 16 }}>
          <div style={{ fontSize: 8, textTransform: "uppercase", letterSpacing: "1.5px", color: C.textMuted, marginBottom: 12 }}>
            Historial ALTTEZ Score
          </div>
...
export default function MatchCenter({ clubId }) {
  ...
  const STORAGE_KEY = `alttez_matchreports_${clubId || "default"}`;
                      ...
                      {matchAthletes.slice(0, 6).map(a => {
                        ...
                        return (
                          <PlayerCardPro
                            ...
                            athlete={a}
                            stats={ps}
                            alttezScore={calcALTTEZScore(ps)}
                            rpe={rpe}
                            historial={historial}
