...
const ALTTEZ_SALT = "alttez_secure_salt_2026";
...
const customStorage = {
  getItem: (name) => {
    ...
    try {
      ...
      if (parsed.state) {
        ...
        const expectedChecksum = secureHash({ ...restState, salt: ALTTEZ_SALT });
  ...
  setItem: (name, value) => {
    ...
    const newChecksum = secureHash({ ...restState, salt: ALTTEZ_SALT });
    const secureValue = {
      ...value,
      state: {
        ...restState,
