 ...
 *  |------------------------------|----------|-------------------------------|
 *  | RPE > 8 Y ALTTEZScore > 7   | critical | Reducir carga 48h post-partido|
 ...
 *
 * Formula de mapeo:
 *   avgScore = mean(ALTTEZScore por partido en matchHistory)
 *   OVR = round(50 + (avgScore / 10) * 49)
 *
 ...
 *   ALTTEZScore avg 0.0 → OVR 50 (minimo, sin partido registrado se usa 65)
 *   ALTTEZScore avg 5.0 → OVR 74-75 (jugador de rendimiento medio)
 *   ALTTEZScore avg 8.0 → OVR 89   (jugador destacado consistente)
 *   ALTTEZScore avg 10.0 → OVR 99  (rendimiento perfecto sostenido)
 *
 ...
 *
 * @param {Object[]} matchHistory - Array de stats de partido (mismo formato que calcALTTEZScore)
...
export function calcOVR(matchHistory) {
  ...
  const avgScore = matchHistory.reduce((sum, m) => sum + calcALTTEZScore(m), 0) / matchHistory.length;
  // Mapear [0, 10] → [50, 99]
 ...
 * rendimiento de partido (ALTTEZScore).
 *
 ...
 * @param {number} alttezScore - ALTTEZ Score del partido, calculado por calcALTTEZScore [0-10]
 * @returns {{ level: "critical"|"warning", message: string, color: string } | null}
 *   null si no hay condicion de alerta (RPE <= 7)
 */
export function getPerformanceAlert(rpe, alttezScore) {
  if (rpe > 8 && alttezScore > 7) {
    return {
      level: "critical",
 ...
 *
 * @param {Object} stats - Estadisticas del partido (mismo formato que calcALTTEZScore)
...
export function generateRecommendations(stats, rpe, pos) {
  const recs = [];
  const score = calcALTTEZScore(stats);
