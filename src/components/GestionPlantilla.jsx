...
// ─────────────────────────────────────────────
// FORMACIONES
// Cada entrada define las posiciones relativas
// de los 11 titulares en el campo (left/top en %)
// ─────────────────────────────────────────────
const FORMATIONS = {
  "4-3-3": [
...
/** Genera URL de avatar DiceBear para un jugador */
const getAvatarUrl = (seed, bg = "059669") =>
  `https://api.dicebear.com/7.x/avataaars/svg?seed=${seed}&backgroundColor=${bg}`;

/** Calcula la edad en años a partir de fecha de nacimiento (ISO string) */
...
/** Devuelve color y texto según estado del jugador */
const getStatusStyle = (status) => ({
  P: { color: PALETTE.green,  label: "Disponible" },
  A: { color: PALETTE.danger, label: "Ausente"    },
  L: { color: PALETTE.amber,  label: "Lesionado"  },
}[status] || { color: PALETTE.textMuted, label: "—" });
